...
static int init_hw_accel_context(video_context_t *video) {
    ...
    // For V4L2 M2M with DRM PRIME: Create frames context but DO NOT initialize
...
int video_init(video_context_t *video, const char *filename, bool advanced_diagnostics, bool enable_hardware_decode) {
    ...
    // PRODUCTION: Initialize interrupt callback to prevent infinite hangs on network streams
            ...
            if (video->use_hardware_decode) {
                ...
                if (video->frame->format == AV_PIX_FMT_DRM_PRIME && video->frame->data[0]) {
                    ...
                    if (drm_desc && drm_desc->nb_objects > 0) {
                        ...
                        // FIX: Calculate total buffer size correctly for YUV420
                        // The obj->size might be 0 or incorrect, so calculate from plane layout
                        size_t calculated_size = 0;
                        
                        // Extract plane layout and calculate proper buffer size
                        for (int layer = 0; layer < drm_desc->nb_layers; layer++) {
                            ...
                            for (int p = 0; p < layer_desc->nb_planes && p < 3; p++) {
                                ...
                                // Calculate the end of this plane
                                size_t plane_height = (p == 0) ? video->height : video->height / 2;
                                size_t plane_end = plane->offset + (plane->pitch * plane_height);
                                if (plane_end > calculated_size) {
                                    calculated_size = plane_end;
                                }
                            }
                        }
                        
                        // Use calculated size if obj->size is invalid
                        if (obj->size == 0 || obj->size < calculated_size) {
                            drm_size = calculated_size;
                            if (frame_count == 1 && hw_debug_enabled) {
                                printf("[ZERO-COPY] Calculated buffer size: %zu bytes (obj->size was %zu)\n", 
                                       calculated_size, (size_t)obj->size);
                            }
                        } else {
                            drm_size = obj->size;
                        }
